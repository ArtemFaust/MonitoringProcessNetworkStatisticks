#!/usr/bin/env python
from PyQt4 import QtGui, QtCore, uic, QtSql, Qt
import sys
...
# Выставляем параметры контекстного меню listWidget_3 и подключаем сигнал клика правой мышки к функции меню
        ...
        self.listWidget_3.connect(self.listWidget_3,
                                                QtCore.SIGNAL("customContextMenuRequested(QPoint)"),
                                                self.listWidget_3ItemRightClicked)
...
# Вставляем сплиттер и добавляем в него виджеты
        self.splitterH = QtGui.QSplitter(QtCore.Qt.Vertical)
...
# Выводим меню в listWidget_3 и соединяем сигнал клика по меню с соотеветсвующей функцией
    def listWidget_3ItemRightClicked(self,QPos):
        ...
        self.connect(menu_item2, QtCore.SIGNAL("triggered()"), self.menuItem2Clicked)
    ...
    def listWidgetItemOnClick(self,item):
        ...
        if ipaddressInterface != None and ipaddressInterface[0][1] != None:
            for data in ipaddressInterface[0]:
                try:
                    listWidget_2Item = QtGui.QListWidgetItem()
...
class processInfoWidget(QtGui.QWidget):
    def __init__(self):
        QtGui.QWidget.__init__(self)
    ...
    def killingProcess(self):
        if self.pushButton_3.text() == "Kill":
            ...
            if rezult == QtGui.QMessageBox.No:
                pass
            elif rezult == QtGui.QMessageBox.Yes:
                process = psutil.Process(int(self.label.text().split("Pid: ")[1]))
                process.kill()
                QtGui.QMessageBox.information(None, "Process killed", "Process killed")
...
class processStatistickWidget(QtGui.QWidget):
    def __init__(self):
        QtGui.QWidget.__init__(self)
        self.spForCpu = []
        self.spForMem = []
